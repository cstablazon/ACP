...
namespace ACP
{
    public partial class frmProductDetails : Form
    {
        ...
        private void tvPopup_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if(profile == "BMRX")
            {
                bmrxID = Convert.ToInt64(selectedNodeID);
            }
            else if(profile == "Privilege")
            {
                privilegeID = Convert.ToInt64(selectedNodeID);
            }
                ...
                if (Id.button == "Create")
                {
                    if (btnCreate.Text == "Create")
                    {
                        if (Id.dt.Rows.Count > 0)
                        {
                            ...
                            {
                                MessageBox.Show("" + barcode + " already exist in Barcode line", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
